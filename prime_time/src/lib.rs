pub mod prime_time_server {
    ...
    pub fn chunk_read(
        ...
        back_buf_end: &mut usize) -> io::Result<usize> {

        buf[..*back_buf_end].copy_from_slice(&back_buf[..*back_buf_end]);
        let read_len = stream.read(&mut buf[*back_buf_end..])?;

        let s = std::str::from_utf8(&buf[..read_len]).unwrap();
        println!("raw read data : {:?}", s);
    ...
    fn reponse(mut stream: TcpStream) {
        println!("==> new connection : {}", stream.peer_addr().unwrap().port());
        ...
        loop {
            ...
            match chunk_read(&mut stream, &mut buf, &mut back_buf, &mut back_buf_end) {
                Ok(chunk_len) => {
                    ...
                    // print buf text
                    let slice_as_string = std::str::from_utf8(&buf[..chunk_len]).unwrap();
                    println!("request text : {:?}", slice_as_string);
                    // println!("request bytes : {:?}", buf);
                    ...
                    for request in v {
                        match is_prime_request(&request) {
                            Some(is_prime) => {
                                ...
                                response_text.push_str(&serde_json::to_string(&response).unwrap());
                    ...
                    println!("back_buf_end : {:?}", back_buf_end);
    ...
    pub mod tests {
        ...
        fn TEST_PARSE_SINGLE_VALID_REQUEST_IGNORE_PROPERTY() {
            let param = "{\"method\":\"isPrime\",\"number\":1,\"ignore_prop\": \"should ignore\"}\n";
            let mut buf = [0; BUF_SIZE];
            buf[..param.len()].copy_from_slice(param.as_bytes());
            let v = parse_request(&buf, param.len());
            assert_eq!(v.len(), 1, "1 valid request");            
...
"#;
        ...
        fn TEST_CHUNK_READ2() {
            ...
            let chunk_len: usize = chunk_read(
                ...
                &mut back_buf,
                &mut back_buf_end).unwrap();
            assert_eq!(&buf[..chunk_len], "{\"method\":\"isPrime\",\"number\":1}\n".as_bytes());
            assert_eq!(back_buf_end, 0);
        }
        #[test]
        fn TEST_CHUNK_READ2_1() {
            let mut buf = [0; BUF_SIZE];
            let mut back_buf = [0; BUF_SIZE];
            let mut back_buf_end: usize = 0;
